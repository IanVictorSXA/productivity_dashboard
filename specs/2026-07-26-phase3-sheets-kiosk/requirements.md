...
- **An event whose ring time has already passed is not created** (user decision, 2026-08-01). If the sync runs at 09:00 and column A holds a `645am` event, that event is skipped: creating it would put a card on the board that rings the instant it appears, for something the user has already done or already missed. The Pi booting late in the day is the normal case here, not an exotic one — a power cut at noon means every morning event is in the past.
  - **The comparison is made when the card is created, not when the cell is parsed.** Group 4's parser is deliberately time-independent: it turns `645am` into a timestamp and says nothing about whether that moment has passed, so the same cell parses identically at any hour. The skip belongs to `sync_day` in Group 5.
  - **Both sides of the comparison are absolute** — the parsed `ring_time` is UTC, and it is compared against "now" in UTC — so the timezone question Group 4 settled does not come back here. An event whose ring time is **exactly** now counts as past; it would ring immediately, which is the thing being avoided.
  - **This applies only to creation, never to deletion.** An event created this morning that has since rung is a *matched* card on later syncs, and § Reconciliation's "leave matched cards untouched" wins. Without this, tapping the manual sync button in the evening would delete the whole day's events for having passed — the exact opposite of what the rule is for.
  - **Tasks are unaffected**, having no ring time. A task stays on the board until its cell leaves column A.
  - Skipped-as-past is logged and counted **separately from malformed**, so the log explains why a cell that is plainly in the sheet has no card. It is a normal outcome, not an error.
  - Accepted consequence: the event never appears that day, and re-tapping manual sync will not bring it back, because it is still in the past. Correcting a mis-typed time in the sheet does bring it back — the edited text is a new key, and if the new time is in the future the card is created.
...
- Declared **only** in the `CREATE TABLE IF NOT EXISTS` statements in `Database.__init__`. **No migration code** — no `ALTER TABLE`, no `PRAGMA table_info` check. User-directed decision: the user drops the old `events`/`tasks` tables on the Pi themselves, so the tables are recreated with the new shape on the next start. The trade-off, accepted knowingly: **any cards in those tables at that moment are lost**, and a stale `productivity.db` that was *not* dropped fails rather than silently working (see § Failure paths). Do the drop while the board is empty — e.g. right after a daily rollover — so nothing of value is discarded.
- **Measured 2026-08-18, and wider than § Failure paths first described**: against old-shape tables the failure is `sqlite3.OperationalError: table tasks has no column named source`, raised from **any** card creation — not only a sync. `get_tuple_to_save()` names the new columns unconditionally, so a hand-made card fails too, and Group 6's guard (which only wraps the sync) does not cover that half. Until the drop is done the dashboard can display existing cards but cannot create new ones. This makes the drop a hard prerequisite of running the Group 5 build at all, rather than a prerequisite of enabling sync.
...
| Individual cell unparseable | Skip that cell, sync the rest, log the row number — and treat the cell's existing card as **still present**, never as deleted. Structural since Group 6: the delete pass diffs against *every non-empty cell*, parsed or not (`SheetRead.cells`), so a cell that stops parsing cannot read as a removed row. |
...
- **Two guards, because the failure can precede the guarded code** (Group 6). `sheets.sync_day()` guards the sync itself; `TaskManager.sync_sheets()` guards reading the config and `import sheets`, which happen before `sync_day` exists — a missing `gspread` in a stale image is a real, observed failure and must not be a backend that will not start.
- **A failure writes `status = 'error'` on that date's `sheet_sync` row**, superseding an earlier `ok` row for the same date: the row means "the last attempt, and how it went", which is what makes it usable as the manual button's feedback. The automatic trigger gates on `status == 'ok'`, so an error row leaves the day retryable — and re-running is idempotent, since matched cards are left alone. If the marker write *itself* fails it is logged and swallowed; a broken database must not become a backend that will not start.
- **The logging has to be visible to count.** Uvicorn configures its own loggers and leaves the root logger empty at WARNING, which silently dropped every `logger.info` in the sync path and stripped the level and timestamp from its warnings. `main.py` sets `logging.basicConfig(level=INFO, ...)` before the `TaskManager()` that runs the startup sync; `sheets.py`'s `__main__` block does the same for `--check`/`--dump`.
- **Status is exposed on `GET /api`** as a small `sheet_sync` object (date, status, detail) so the failure is diagnosable without SSH-ing into the Pi to read logs. The manual sync button (3f) is the only consumer of this field in this phase — it uses it for last-result feedback. No other UI rendering of sync state is in scope.
  - It reports the **last attempt, held in memory**, not a read of the `sheet_sync` table — so the failures that never reach the table (a failed `import sheets`, a failed `get_date()`, a failed marker write) are still visible, and so is the switched-off case, which has no row by definition.
  - `status` is one of `disabled`, `ok`, `skipped` (already synced today), `error`. **`disabled` is a reported state**: it is how the frontend knows to hide the manual sync button rather than show a dead one.
  - A `skipped` result carries the earlier run's counts in its detail, so a same-day restart still publishes what today's sync actually did.
