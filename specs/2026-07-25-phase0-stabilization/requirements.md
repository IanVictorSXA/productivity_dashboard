...
- **Regression checks**: introduce `pytest` for the backend (currently no test framework anywhere in the repo). Add it to `backend/requirements.txt`. Tests exercise `TaskManager.process_command` and `Database` directly (no HTTP layer needed — `main.py` is a thin FastAPI wrapper). Frontend gets no new test tooling in this phase; audit findings there are covered by the manual walkthrough in `validation.md`.
  - **✅ COMPLETE (Group 1)**: Pytest harness + 51 comprehensive tests (48 passing, 3 xfailed for known Timer.edit() bug). Covers Task, Event, Duration/Stopwatch, and Timer classes with creation, command handling, and database persistence verification. All fixtures use temp files, never touching production DB.
...
1. **Shutdown crashes the backend.** `App.tsx` `handleShutdown` sends `{ command: "close", current_time: ... }` on every click of the header "X" button. `TaskManager.process_command`'s `match msg.command` has no `"close"` case, so it falls into `case _: raise NotImplementedError(...)`.
   - **Decision**: `"close"` should be a no-op that does not crash — add a `case "close":` branch that handles it without throwing. The button does not trigger actual process shutdown; it's only a signal that the frontend's UI is closing. Actual process lifecycle (Pi reboot, container stop) is handled outside the app.
...
4. **Day-rollover + shutdown interaction is unverified.** `Database.__init__` calls `deleteAll()` when `date_id.txt`'s stored date differs from today, wiping all tables (including the persistent Work/Misc/Waste stopwatches). Combined with finding #1 (shutdown crashes before the delete-then-close sequence in `App.tsx` completes), it's unconfirmed whether a normal end-of-day shutdown leaves the DB in the state the next startup expects. Needs explicit audit under 0a/0d.
   - **✅ RESOLVED (Group 5)**: Confirmed a real bug — `handleShutdown` was deleting every card on every shutdown, independent of the day-rollover check, so same-day restarts lost all state. Fixed by making shutdown a no-op besides `close`; card clearing is now exclusively the day-rollover's job. See `plan.md` Group 5.
...
6. **Dismissed events re-ring after reload (found during Group 5 validation).** `Event.get_ApiEvent()` never returned `alerting`, and the `events` table had no `completed` column — so `stop_ring` clearing `alerting` in memory/DB wasn't enough to keep an event silenced, since `ring_time` stays in the past forever and the frontend's countdown-check re-fires `ring` on every load. Directly relevant to 0c and 0d.
   - **✅ RESOLVED (Group 5 addendum)**: Added `completed` column to `events` table; `stop_ring` now persists it; `edit` resets `alerting`/`completed` when the ring time changes; frontend loads `completed` from the API instead of hardcoding `false`. See `plan.md` Group 5.
7. **Rung timers/stopwatches resume as "running" after reload (found during Group 5 validation).** `Timer.ring()`/`Duration.ring()` set `alerting = True` but never `paused = True`. `get_ApiDuration()` derives `started_at` from `paused`, so a rung-but-unpaused card reloads reporting `started_at != None` — the frontend treats it as actively running with elapsed already past `total_time` and instantly re-fires `ring()`. Directly relevant to 0b and 0d.
   - **✅ RESOLVED (Group 5 addendum)**: `Timer.ring()`/`Duration.ring()` now also set and persist `paused = True`, freezing the running state. See `plan.md` Group 5.
